...
const loadPage = async (slug: string) => {
  if (!slug) {
    console.warn('[EventPage] No slug provided')
  ...
  resetState()
  loading.value = true
  console.log('[EventPage] Loading event:', slug)
  ...
  try {
    ...
    if (eventId) {
      ...
      const eventsResponse = await eventsApi.list({ status: 'approved' })
      const events = eventsResponse.results || eventsResponse || []

      const found = events.find(e => e.slug === slug || slug.includes(e.slug))
      ...
      event = await eventsApi.getById(found.id)
    }
    ...
    errorMessage.value = 'Échec du chargement de l\'événement.'
  }
  finally {
    loading.value = false
  }
...
watch(
  () => route.params.slug,
  (newSlug) => {
    console.log('[EventPage] Route slug changed to:', newSlug)
    if (newSlug) {
      loadPage(newSlug)
    }
  }
...
onMounted(() => {
  console.log('[EventPage] Mounted, route.params.slug:', route.params.slug)
            ...
            <VCardText class="pa-6">
              ...
              <div class="quantity-selector mt-4">
                ...
                <div class="quantity-controls">
                  <VBtn
                    ...
                    size="small"
                    variant="outlined"
                    :disabled="quantity <= 1 || !isValidQuantity"
                ...
                <p v-else class="text-caption text-error mt-1">
              ...
              <div class="booking-summary mt-4">
                ...
                <div class="text-medium-emphasis text-body-2">
                  {{ authStore.role === 'attendee' ? 'Prêt à ajouter au panier' : 'Connectez-vous en tant qu\'utilisateur pour réserver' }}
                </div>
              </div>
